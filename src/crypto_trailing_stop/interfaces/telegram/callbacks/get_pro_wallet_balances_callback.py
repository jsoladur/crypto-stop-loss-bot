    ...
    if is_user_logged:
        try:
            async with await operating_exchange_service.get_client() as client:
                account_info = await operating_exchange_service.get_account_info(client=client)
                portfolio_balance = await operating_exchange_service.retrieve_porfolio_balance(
                    account_info.currency_code, client=client
                )
                trading_wallet_balances = await operating_exchange_service.get_trading_wallet_balances(client=client)
            message = messages_formatter.format_trading_wallet_balances(
                account_info, trading_wallet_balances, portfolio_balance.total_balance
            )
            await callback_query.message.answer(text=message)
        except Exception as e:
            logger.error(f"Error fetching get the Exchange Wallet balances: {str(e)}", exc_info=True)
            await callback_query.message.answer(
                "⚠️ An error occurred while fetching the Exchange Wallet balances. "
                + f"Please try again later:\n\n{html.code(format_exception(e))}"
            )
    else:
        await callback_query.message.answer(
            "⚠️ Please log in to get the current the Exchange Wallet balances!",
