            ...
            if limit_sell_order_guard_metrics_list:
                ...
                for idx, metrics in enumerate(limit_sell_order_guard_metrics_list):
                    crypto_currency, fiat_currency = (
                        metrics.limit_sell_order.symbol.split("/")
                    )
                    answer_text += (
                        f"- 🚀 {html.bold('Sell Limit Order')} :: 💰 {metrics.limit_sell_order.order_amount} {crypto_currency}, "
                        ...
                        + f"    * 🛡️ {html.bold('Safeguard Stop Price: ' + str(metrics.safeguard_stop_price) + ' ' + fiat_currency)}"
                    )
                    if idx + 1 < len(limit_sell_order_guard_metrics_list):
                        answer_text += "\n\n"
            await callback_query.message.answer(
                answer_text, reply_markup=keyboards_builder.get_home_keyboard()
            )
