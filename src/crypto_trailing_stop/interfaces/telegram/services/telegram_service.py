...
class TelegramService:
    ...
    async def perform_successful_login(self, login: LoginDto, userinfo: dict[str, Any]) -> None:
        ...
        state = await self._session_storage_service.get_or_create_fsm_context(
            bot_id=login.tg_bot_id, chat_id=login.tg_chat_id, user_id=login.tg_user_id
        )
        if not state:
            raise ValueError(f"State not found for chat_id={login.tg_chat_id} and user_id={login.tg_user_id}")
        await self._session_storage_service.set_user_logged(state=state, userinfo=userinfo)
