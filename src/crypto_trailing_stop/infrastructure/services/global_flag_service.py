...
class GlobalFlagService:
    ...
    async def find_all(self) -> list[GlobalFlagItem]:
        ...
        for current in GlobalFlagTypeEnum:
            persisted = next(
                filter(
                    lambda n: GlobalFlagTypeEnum.from_value(n.name) == current,
                    flags,
                ),
                None,
            )
            ret.append(
                GlobalFlagItem(
                    name=current, value=persisted is None or persisted.value is True
                )
            )
        return ret
    ...
    async def toggle_by_name(self, name: GlobalFlagTypeEnum) -> GlobalFlagItem:
        ...
        ret = GlobalFlagItem(
            name=GlobalFlagTypeEnum.from_value(global_flag.name),
