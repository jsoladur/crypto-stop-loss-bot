...
class TrailingStopLossTaskService(AbstractTradingTaskService):
    ...
    async def _handle_single_sell_order(
        ...
        *,
        ...
        if sell_order.stop_price < new_stop_price:
            logger.info(
                f"Updating order {repr(sell_order)} to new stop price {new_stop_price} {sell_order.symbol}."
            )
    ...
    async def _calculate_max_and_min_buy_order_amount_by_symbol(
        ...
        *,
        ...
        sell_order_symbols = set(
            [sell_order.symbol for sell_order in opened_stop_limit_sell_orders]
        )
        ...
        for sell_order_symbol in sell_order_symbols:
            if (
                sell_order_symbol in opened_buy_orders_by_symbol
                and opened_buy_orders_by_symbol[sell_order_symbol]
            ):
                max_buy_order = pydash.max_by(
                    opened_buy_orders_by_symbol[sell_order_symbol],
