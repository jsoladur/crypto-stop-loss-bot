...
OUT_OF_SAMPLE_MONTHS_BACK = 6
MIN_ENTRIES_PER_WEEK = MIN_TRADES_FOR_STATS / (DEFAULT_MONTHS_BACK * 4)  # ~2 trades per week
DECENT_WIN_RATE_THRESHOLD = 70.0

MIN_VOLUME_THRESHOLD_STEP_FIRST_ITERATION = 0.5
...
MIN_VOLUME_THRESHOLD_VALUES_FOR_FIRST_ITERATION = [
    ...
    for min_volume_threshold in np.arange(0.5, 2.5, MIN_VOLUME_THRESHOLD_STEP_FIRST_ITERATION).tolist()
]
MAX_VOLUME_THRESHOLD_VALUES_FIRST_ITERATION = [
    round(max_volume_threshold, ndigits=2)
    for max_volume_threshold in np.arange(2.5, 4.0, MAX_VOLUME_THRESHOLD_STEP_FIRST_ITERATION).tolist()
]
