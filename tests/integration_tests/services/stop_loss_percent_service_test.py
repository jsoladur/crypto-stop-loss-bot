...
async def should_set_stop_loss_percent_service_properly(
    ...
    stop_loss_percent_item_list = await stop_loss_percent_service.find_all_stop_loss_percent()
...
def _prepare_httpserver_mock(
    ...
    favourite_crypto_currencies = set(faker.random_choices(["BTC", "ETH", "SOL", "XRP", "ADA", "DOT"], length=3))
    httpserver.expect(
