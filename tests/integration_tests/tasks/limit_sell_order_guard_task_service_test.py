...
def _prepare_httpserver_mock(faker: Faker, httpserver: HTTPServer, bit2me_api_key: str, bik2me_api_secret: str) -> None:
    opened_sell_bit2me_order = Bit2MeOrderDtoObjectMother.create(
        side="sell", order_type="limit", status=faker.random_element(["open", "inactive"])
    )
    ...
    httpserver.expect(
        Bit2MeAPIRequestMacher(
            ...
            query_string=urlencode(
                {"direction": "desc", "status_in": "open,inactive", "side": "sell", "orderType": "limit"}, doseq=False
            ),
    ...
    tickers = Bit2MeTickersDtoObjectMother.create(
        symbol=opened_sell_bit2me_order.symbol, close=opened_sell_bit2me_order.price * 0.2
    )
    ...
    for _ in range(number_of_trades):
        buy_trades.append(
            Bit2MeTradeDtoObjectMother.create(
                ...
                price=opened_sell_bit2me_order.price * 0.5,
                amount=opened_sell_bit2me_order.order_amount / number_of_trades,
            )
        )
